...
begin
    # N = length(df_list); #number of assets
    sampleRtrns = zeros(N,length(df_list[1]."close")-1) #matrix containing the daily returns of each asset in each row.
    assetShiftedRtrns = Array{Float64,2}(undef,N,length(df_list[1]."close")-1); #same as up here but then making mean = 0

    for i = 1:N
        df = df_list[i]
        closes = df."close"
        returns = log.(closes[2:end]./closes[1:end-1])
        sampleRtrns[i,:] = returns
        gemiddelde = mean(returns)
        shiftedRtrns = returns-repeat([gemiddelde],length(returns))
...
begin
    V = VG_Params2MRet([getVGParams(pars[i,:]...,1,20) for i = 1:N], M)
end
...
begin
    ...
    lc = [0]; uc = [0];
    fun(x) = -GAPSORTED(x,Rtrns,RtrnsSorted)
    ...
    println("Best weights: $w_optimgap \nWith gap: $gap_optim")
end
...
begin
    ...
    w_optimgap_short = Optim.minimizer(res2)
    gap_optim_short  = GAP(w_optimgap_short,Rtrns)
    println("Best weights: $w_optimgap_short \nWith gap: $gap_optim_short")
end
println("\n---------------------------------------------------------\n")
