...
begin
    ...
    for i = 1:N
        df_list[i] = DataFrame(CSV.File("data/DJ2000/"*filenames[i],delim=","))
    end
...
# let's find 1999-12-31 (DJIA goes down here)
y2k_pos = findall(x->x==Date(1999,12,31),df_list[1]."Date")[1] #position 896
...
# open("data/Rtrns2000.txt", "w") do io
#     writedlm(io, Rtrns)
# end
Rtrns = readdlm("data/Rtrns2000.txt",Float64)

# w_optimvar = getMinVaRWeights(Rtrns,0.95,false)
w_optimvar = vec(readdlm("data/w_optimvar2000.txt",Float64))
# w_optimvar_short = getMinVaRWeights(Rtrns,0.95,true)
w_optimvar_short = vec(readdlm("data/w_optimvar2000_short.txt",Float64))
# w_optimcvar95 = getMinCVaRWeights(Rtrns,0.95,false)
w_optimcvar95 = vec(readdlm("data/w_optimcvar95-2000.txt",Float64))
# w_optimcvar95_short = getMinCVaRWeights(Rtrns,0.95,true)
w_optimcvar95_short = vec(readdlm("data/w_optimcvar95-2000_short.txt",Float64))
# w_optimcvar99 = getMinCVaRWeights(Rtrns,0.99,false)
w_optimcvar99 = vec(readdlm("data/w_optimcvar99-2000.txt",Float64))
# w_optimcvar99_short = getMinCVaRWeights(Rtrns,0.99,true)
w_optimcvar99_short = vec(readdlm("data/w_optimcvar99-2000_short.txt",Float64))


open("data/w_optimvar2000.txt", "w") do file
    writedlm(file, w_optimvar)
end
open("data/w_optimvar2000_short.txt", "w") do file
    writedlm(file, (w_optimvar_short))
end
open("data/w_optimcvar95-2000.txt", "w") do file
    writedlm(file, (w_optimcvar95))
end
open("data/w_optimcvar95-2000_short.txt", "w") do file
    writedlm(file, (w_optimcvar95_short))
end
open("data/w_optimcvar99-2000.txt", "w") do file
    writedlm(file, (w_optimcvar99))
end
open("data/w_optimcvar99-2000_short.txt", "w") do file
...
#     open("data/w_optimgap$nummer-2000.txt", "w") do file
#         writedlm(file, (five_γ[i]))
#     end 
#     five_γ_short[i] = getMinConicWeights(Rtrns,a[i],true)
#     open("data/w_optimgap$nummer-2000_short.txt", "w") do file
#         writedlm(file, (five_γ_short[i]))
#     end 
# end
for i = 1:6
    nummer = a[i] 
    five_γ[i] = vec(readdlm("data/w_optimgap$nummer-2000.txt",Float64))
    five_γ_short[i] = vec(readdlm("data/w_optimgap$nummer-2000_short.txt",Float64))
end
...
h = 7
begin
    indexMPT_change = (-indexMPT[1:end-h]+indexMPT[h+1:end])./indexMPT[1:end-h]
    indexvar_change = (-indexvar[1:end-h]+indexvar[h+1:end])./indexvar[1:end-h]
    indexcvar95_change = (-indexcvar95[1:end-h]+indexcvar95[h+1:end])./indexcvar95[1:end-h]
    indexcvar99_change = (-indexcvar99[1:end-h]+indexcvar99[h+1:end])./indexcvar99[1:end-h]
    indexfive_γ_change = zeros(size(indexfive_γ,1)-h,size(indexfive_γ,2))
    for i = 1:6
        indexfive_γ_change[:,i] = (-indexfive_γ[1:end-h,i]+indexfive_γ[h+1:end,i])./indexfive_γ[1:end-h,i]
    end
end
...
function plotall()
    ...
    if k == -365
        PlotlyJS.savefig(p, "plots/2000 crash/2000all-365.html")
    elseif k == -30
        PlotlyJS.savefig(p, "plots/2000 crash/2000all-30.html")
    else
        PlotlyJS.savefig(p, "plots/2000 crash/2000all.html")
    end
end
plotall()

function plotall_change()
    crashdate = df_list[1]."Date"[y2k_pos+k:end-h]
    one =   PlotlyJS.scatter(;x=crashdate, y=indexfive_γ_change[:,1], mode="lines",name = "Conic γ=$(a[1])")
    two =   PlotlyJS.scatter(;x=crashdate, y=indexfive_γ_change[:,2], mode="lines",name = "Conic γ=$(a[2])")
    three = PlotlyJS.scatter(;x=crashdate, y=indexfive_γ_change[:,3], mode="lines",name = "Conic γ=$(a[3])")
    four =  PlotlyJS.scatter(;x=crashdate, y=indexfive_γ_change[:,4], mode="lines",name = "Conic γ=$(a[4])")
    five =  PlotlyJS.scatter(;x=crashdate, y=indexfive_γ_change[:,5], mode="lines",name = "Conic γ=$(a[5])")
    six =   PlotlyJS.scatter(;x=crashdate, y=indexfive_γ_change[:,5], mode="lines",name = "Conic γ=$(a[6])")
    seven =   PlotlyJS.scatter(;x=crashdate, y=indexMPT_change, mode="lines",name = "Mean-Variance")
    eight = PlotlyJS.scatter(;x=crashdate, y=indexvar_change, mode="lines",name = "Mean-VaR")
    nine = PlotlyJS.scatter(;x=crashdate, y=indexcvar95_change, mode="lines",name = "Mean-CVaR 95%")
    ten = PlotlyJS.scatter(;x=crashdate, y=indexcvar99_change, mode="lines",name = "Mean-CVaR 99%")
    # eleven = PlotlyJS.scatter(;x=crashdate, y=DJI_val[y2k_pos+k:end], mode="lines",name = "DJIA",opacity=0.25)
    p = PlotlyJS.plot([one,two,three,four,five,six,seven,eight,nine,ten])
    if k == -365
        PlotlyJS.savefig(p, "plots/2000 crash/2000all-365_change.html")
    elseif k == -30
        PlotlyJS.savefig(p, "plots/2000 crash/2000all-30_change.html")
    else
        PlotlyJS.savefig(p, "plots/2000 crash/2000all_change.html")
    end
end
plotall_change()
...
function plotall_short()
    ...
    p = PlotlyJS.plot([one,two,three,four,five,six,seven,eight,nine,ten,eleven])
    if k == -365
        PlotlyJS.savefig(p, "plots/2000 crash/2000all-365_short.html")
    elseif k == -30
        PlotlyJS.savefig(p, "plots/2000 crash/2000all-30_short.html")
    else
        PlotlyJS.savefig(p, "plots/2000 crash/2000all_short.html")
    end
end
plotall_short()
